...
class ProfilePane extends StatefulWidget {
  ...
  ProfilePane({Key? key, required this.user, required this.currentUser})
      : super(key: key);
...
class _ProfilePaneState extends State<ProfilePane> {
  ...
  Future<void> _checkFollowingStatus() async {
    ...
    setState(() {
      isFollowing = followedUsers.contains(widget.user);
    });
  }

  // Add or remove the user from the followed_users table
  Future<void> _toggleFollow() async {
    if (isFollowing) {
      SQLService().unfollowUser(widget.currentUser.id, widget.user.id);
    } else {
      // If not following, add to followed_users
      SQLService().followUser(widget.currentUser.id, widget.user.id);
    }
        ...
        children: [
          Row(
            children: [
              ...
              IconButton(
                iconSize:
                    40, // This controls the overall size of the IconButton widget
                icon: CircleAvatar(
                  radius:
                      40, // This controls the size of the CircleAvatar (should be half of the IconButton size)
                  backgroundImage: NetworkImage(
                    widget.user == widget.currentUser
                        ? widget.currentUser.profileImage ??
                            SQLService.defaultProfileImage
